...
class InvoiceRepository(Repository[schemas.InvoiceSchema]):
	...
	def add(self, invoice: schemas.InvoiceSchema, s3_path: str) -> str:
		invoice_model = models.InvoiceModel(**invoice.model_dump(), s3_path=s3_path)
		self.session.add(invoice_model)
		self.session.commit()
		self.session.refresh(invoice_model)
		LOGGER.info("Invoice %s add to database. Metadata: %s", invoice_model._id, invoice)
		return invoice_model.file_id
	...
	def get(self, id_: str) ->  schemas.InvoiceSchema:
		invoice_model = self.session.exec(
			sqlmodel.select(models.InvoiceModel.file_id == id_)
		).one()
	...
	def get_all(self, limit: int = 10) -> list[schemas.InvoiceSchema]:
		...
		invoices = [schemas.InvoiceSchema(**invoice_model.model_dump()) for invoice_model in invoice_models]
